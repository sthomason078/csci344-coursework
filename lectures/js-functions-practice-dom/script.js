...
const createTodoItemHTML = (text, className) => `<li class="${className}">${text}</li>`;
...
const addTodo = () => {
    ...
    if (cleanText !== "") {
        const todoItemHTML = createTodoItemHTML(text);
        const listEl = document.querySelector("#todoList");
        listEl.insertAdjacentHTML("beforeend", todoItemHTML);
        inputEl.value = "";
    }
}
