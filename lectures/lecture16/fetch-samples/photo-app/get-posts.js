...
export async function getPosts() {
    ...
    const token = await getAccessToken(rootURL, "sthoma12", "password");
    ...
    const myPosts = posts.filter(post => post.user.username === "sthoma12");

    // print a dump of the first post:
    console.log(JSON.stringify(myPosts[0], null, "   "));

    // print all of the post ids:
    myPosts.forEach((post) => console.log(post.id));

    // print all of the users associate with the posts in your feed:
    myPosts.forEach((post) => console.log(post.user.username));
}
